...
\paragraph{Experiments analysis}
In run 1 the robot grasps object \ttt{o3} although there was \ttt{o4} on top of it. This is because during the execution of \ttt{(push\_dir1 o0)}  action the robot collided with \ttt{o4}, corrupting in this way the scene.  In runs 1, 4 and 5 the optimal plan does not return to push object \ttt{o0} as a solution but it returns to push either \ttt{o5} or \ttt{o3}, this also is because of the segmentation (at that iteration that system detected that it was not possible pushing away \ttt{o0}). 
...
Sometimes because the object detection was not good enough, the grasping pose for \ttt{o4} was marked as unfeasible and the system replanned. This was then fixed by successive segmentations.
