...
\item probabilistic planners\DM{.}
\end{enumerate}
...
\textbf{Probabilistic planning} is a planning technique which considers that the environment is not deterministic but probabilistic. So the actions have a \DMC{certain} probability to obtain a certain state, and given an initial and \DMC{final state}{goal,} the planner  finds the solution path with the highest reward, which depends also on the probability. \DMC{A well known example on which this kind of planners are build on is the Markov Decision Process}{Probabilist problems problems are usually formalized as Markov Decision Processes}.
...
The problem involves a big amount of uncertainty due to the interaction of the robot with the environment. When the robot \DMC{will}{} interacts with the objects\DM{,} it is very hard to predict correctly the position of the object after the execution, that is the next state\DMC{, this is a crucial problem which should be considered}{}. \DMC{With a probabilistic planner, the planner will take into account what object has been moved and it will update the state obtaining a set of states, each one with a certain probability.}{A probabilistic planner considers the probability associated with every effect to predict the state after executing an action...} The probability also has to be \DMC{modelled}{specified or learned for each type of object} \DMC{and it is highly depending on the form of the object}.
...
To conclude, \DM{in many problems it is more efficient to replan with a deterministic planner rather than directly using a probabilitic planner.} \DMC{a replanner makes more probabilistic problems practically solvable. In the other hand it suffers to be less robust than a probabilistic one}{}. 
...
Taking into account such considerations and that, except for rare cases, our planning problem is not \textit{probabilistic interesting}, the problem has been thought to be solved \DMC{thanks to}{by} a deterministic planner.

A hierarchical planner would be a good choice if the problem \DMC{presents}{presented} some hierarchies, for instance in the case the goal \DMC{is}{was} to clear several tables. Since the problem is about cleaning a single table \DMC{the problem can be easily modelled through a classic planner and it is straightforward}{it is more straightforward to use a classical planner}.

The planner chosen \DMC{to develop the work is}{was} the \textbf{Fast Downward} planner \citep{helmert2006fast}, a very well know classic one. 
This planner is feature-wise complete, stable and fast in solving planning problem\DM{s}.

\section{Symbolic Predicates}
\DMcomment{Should be two different sections: formulation and predicates}

\DMC{Once the planner has been chosen, the planning problem has to be formulated accordingly to the capabilities of the planner. Since we chosen a classic one, the problem is deterministic and it is generally}{The problem is} formulated as a $6$-tuple $\Pi=\langle S^d, s_o^d, G^d, A^d, T^d, c^d \rangle$, where:
\begin{itemize}
...
\item $T^d(a,s) :$ \DM{$ S \times A \times S$} is a deterministic transition function\DMC{ for all actions $a \in A^d(s)$ and states $ s \in S^d$}{};
\item $c^d(a)$ is the cost to apply action $a$.
\end{itemize}
\DMcomment{Why do you put a $d$ on everything?}
The plans, or trajectories\DMcomment{why trajectories? You get plans (or action sequences) with a planner.}, $\tau_i$ are sequences of actions applicable from the initial state until the goal state. The cost of a trajectory $C(\tau_i)$ is the sum of the cost of the actions of the trajectory $C(\tau_i) = \sum_{a \in \tau} c^d(a)$. The optimal solution is the solution with less cost: $\tau^* = \min_{\tau_i} c(\tau_i)$.
In this work we \DMC{did}{do} not specify \DMC{any}{specific} cost \DMC{to}{for} the actions, so the planner \DMC{will return as solution the one with less number of}{returns the plan with fewer} actions.
\DMcomment{You don't use costs now for anything?}


\DMcomment{The predicates section should start here}
The \textit{Fast Downward} planner needs the problem to be formulated in \DMC{the common format of PDDL}{} (\textit{Problem Domain Description Language}) \citep{pddl}. In this section the symbolic predicates that have been considered in order to solve the problem are described.

The task this thesis faces is a common task done by humans, who think in order to find a feasible sequence of actions. Such a sequence is normally composed of actions that avoid the collision between the manipulated objects and the other ones, whenever possible. To do this we, as humans, think on what is going to happen if we manipulate an object in a certain way. The design of the problem has been inspired by such a reasoning way and therefore we will handle carefully the objects avoiding collisions between them. The reasoning part is entrusted to the planner, which needs a coherent problem description through symbolic predicates. \DMcomment{Too long... Say something short such as: Symbolic predicates are added so that the planner can reason about collisions and geometrical constraints.}
...
This happens when all the \ttt{ik\_unfeasible} predicates for an object are set to true. When this situation occurs that object is removed from the goal. \DMcomment{Say that it is removed from the goals so that the rest of the goal can be completed.}

\paragraph{PDDL}
For clarity purposes, the PDDL syntax of the described actions is \DMC{here reported}{shown here}. 
...
\mbox{}\DMcomment{ Why is there an mbox here?}

\DMC{How the predicates are computed will be discussed in detail in the next chapter.}{}
