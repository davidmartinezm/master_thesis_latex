...
Many manipulation planning approaches \DMC{(see \citep{PlanningAlgorithms} for an overview)}{\citep{PlanningAlgorithms}} assume that the task can be treated as a geometric problem\DMC{,}{} with the goal to place the objects in their desired positions. Planning is essentially done with a mixture \DMcomment{can you say mixture in this context?} of symbolic and geometric states\DMC{, this
requires a set of symbolic predicates that correspond to geometric
relationships. Cause generating geometric paths is very time consuming, these hybrid planners could be too slow to be used in real applications.}{. They require to obtain the symbolic predicates that represent geometric features, which are very time consuming. Therefore, these hybrid planners can be too slow in real applications.  }
...
Dogar and Srinivasa \cite{Dogar2011} proposed a framework for planning \DMC{in}{with} cluttered scenes using a library of actions inspired by human strategies. They designed \DMC{a new planner}{a planning system} that decides which
objects to move, the order\DMC{ to move them}{}, where to move them, and \DMC{it chooses the manipulation actions
to use on these objects,}{the appropriate manipulation actions.} \DMC{and}{Moreover,} it accounts for the uncertainty
in the environment all through this process. 
The planning system first attempts to grasp the goal object, \DMC{than}{and} if it is not possible\DM{,} it identifies what is the object that \DMC{obstacles}{prevents} the action\DMC{, then such an object is added}{and adds it} to a list of \DMC{the}{} objects that have to be moved. \DMC{They move}{Afterwards,} those objects \DM{are moved} in whatever position \DMC{such}{} that makes the goal feasible. 
Their work is the most similar to our, but their planning system cannot be directly applied to a table clearing task. The goal is a single object at a time, then to grasp another object they need to replan. The planning system proposed in this thesis is able to perform more complex plans. \DMcomment{Our approach performs better with goals that involve more than one object. We plan sequence of actions considering all objects in the goal. The actions they use to move objects that were in the way may actually hinder future goals.}
...
The idea is to use a high-fidelity physics simulation to predict the effects of actions and a hand-built mapping from geometric to symbolic states. Planning is conducted by a forward search, \DMC{and}{} the effects of actions \DM{are} determined by simulating them, and then \DMC{using}{} the mapping \DM{is used} to update the symbolic state. \DM{However,} Their method \DMC{is promising, by the way the integration of a simulator into the planning system is not straightforward since it}{} requires to know the physic of the manipulated objects \DM{to simulate them}. Moreover the authors didn't test\DMC{ed}{} their planning system with a complex scene like the one\DM{s} faced in this thesis.
Our planning system doesn't use any simulator, instead it \DMC{uses a simple}{relies on a} prediction algorithm to \DMC{understand}{represent} how the objects \DMC{should}{can} be manipulated, leading to a\DMC{n easy implementation and a fast execution in the planning step}{faster and easier to implement solution}. 
...
In \citep{coelhoplanning} the authors address a problem similar to the one of this thesis. The authors coupled \DMcomment{can you use couple in this context?} together pushing and grasping actions for a table manipulation task.
%For pushing they used a similar strategy to \citep{mericli2013achievable}.
They use the concept of reachability \citep{vahrenkamp2013robot} \DMC{for each kind of action}{} to \DMC{quickly}{} exclude impossible poses of the gripper \DMC{of}{at} the planning stage, creating a reliable plan suitable for real-time operation. The authors model the planning problem through \DMC{MDP (Markov Decision Process)}{a Markov Decision Process (MDP)}, discrediting \DMcomment{discretizing?} the world in grid cells and assigning \DMC{to}{} each one a push and grasp vector defined by the reachability concept. Unfortunately the authors were not able to test the grasp skill with their planning system. \DMcomment{Their advantage is that they plan a trajectory level so they can consider more details. In contrast, we plan at an action level, so we can consider more complex goals involving several objects, and will optimize the sequence of actions for completing the whole task (they only consider single-object tasks).}
Our approach skips the model\DMC{l}{}ing of the probability by considering to replan after each action execution, this eases our planning phase. \DMcomment{Don't talk about the replan in this case. Simply because probabilities are not useful for our case we don't replan, but maybe they are useful for their case as they plan at a lower level (trajectory level.)} Moreover, while their method needs to be adapted to each robot, to built a reachability map, our method can be directly integrated in any robotic manipulator. 
...
Machine learning techniques also have been applied to task planning. \DMC{To plan complex manipulation tasks,
robots need to reason on a high level.} \DM{As} Symbolic planning\DMC{, however,}{} requires knowledge about the preconditions and effects
of the individual actions. \DMcomment{Change order: first symbolic planning requires knowledge, then machine learning can be used for that.}
In \citep{abdo2013learning} the authors proposed a\DMC{ practical}{n}
...
With \DM{just a} few demonstration the \DMC{authors proposed a}{} method \DMC{to teach the robot the}{learns the} preconditions and effects of \DMC{individual}{} actions. Their method furthermore enables the robot to combine the learned actions by means of planning to solve new tasks that are more complicated than the learned individual actions. \DMcomment{I don't understand this last part}
...
Although this work looks promising\DM{,} the problem of this approach is that it moves the problem from identifying the right predicates to describe the problem to identifying the right features that let a correct learning of the predicates. \DMcomment{Why is this a problem?}

In \citep{Dearden2014355} Dearden and Burbridge proposed an approach for planning robotic manipulation tasks which uses a learned mapping between geometric states and logical predicates. \DM{First,} The planner \DMC{first}{} plans symbolically, then \DMC{applies}{} the mapping \DM{is applied} to generate geometric positions that are then sent to a path planner. They try to fit a probability distribution function to the geometric states to map them to a symbolic state. \DMcomment{I did not understand this one. They plan symbolically and then with a trajectory planner, but what is this probability distribution for? To generate the input for the symbolic planner?} This work \DMC{sounds promising but the cluttered scene this thesis is going to treat is quite complex to be treated with such a method}{cannot tackle complex scene such as cluttered objects}. Moreover the same authors applied that technique \citep{dearden2013approach} \DMC{for}{to} a simple task \DMC{which involves}{that involved} putting some cups \DMC{(singulated between themselves)}{} onto a tray. \DMC{Although this approach does not involve any hand built mapping function and the results are good, their approach}{Their results are good but it} is very time consuming for real applications. The hand-built function still have a good benefit in terms of computational cost, although they need to be adapted to the problem. Despite this also learning the predicates requires to be adapted to the problem. 
\DMcomment{I will read this paper later, and give you more comment on it.}
...
\mbox{} \DMcomment{What is this mbox for?}

Compared to the state of the art\DMC{ this thesis proposes}{, we propose} a planning system for \DMC{table}{} clearing \DMC{tasks of}{} cluttered objects\DMC{,}{.} \DMC{it}{Our approach} plans \DMC{completely}{} at a symbolic level, \DM{which is} efficient\DMC{ly}{} and \DMC{it}{} is low time consuming  (the time to get a plan is usually less than $0.5$ seconds). The fast computation of the plan is able to make this system suitable for replanning, overcoming in this way \DMC{the lack of probability}{} \DMcomment{probability is simply not useful enough so replanning is better and faster, but dont need to say it here} and geometric information.  \DMC{Moreover no other works have been found that face the table clearing of}{As far as we know, previous approaches haven't tackled very} cluttered \DMC{objects problem}{scenes}, such as the one in Figure \ref{fig:example_scene}. \DMC{It will be shown}{We will also show} that the lack of \DM{complete} geometric constraints \DMC{includes}{introduces} some limitations to the system\DM{,} but \DMC{it plans still efficiently}{the general results obtained are good and the planning phase is very efficient}.  
