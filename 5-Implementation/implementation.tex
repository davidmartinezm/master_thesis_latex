...
In this chapter the \DMC{design of the software}{software design} and the  external libraries used are briefly described. 

The code has been implemented \DMC{in}{using C++and } the ROS framework (Robot Operating System) \citep{ROS}\DMC{ using \ttt{C++} as programming language}{}. 
\DM{Moreover, }The external libraries used are \DM{the} Point Cloud Library (PCL) \citep{PCL}, an open source library which provides a wide array of tools for 3D perception, and the Flexible Collision Library (FCL) \citep{pan2012fcl}, an open source  library for collision detection.

The algorithm \DMC{is mainly based}{relies heavily} on the PCL library \DMC{which is used}{} to do the following operations: filtering, segmentation, plane estimation, principal component analysis, projections onto the table plane and convex hulls.
The FCL library was used only for collision detection between the convex hulls of the objects and the gripper as well. 

The planner used is the Fast Downward planner \citep{helmert2006fast}\DMC{. To get a plan the binary file of planner is called giving as inputs a domain and a problem description in}{which uses the } PDDL \DMC{syntax}{ language to define the domain and problem description}.
...
\paragraph{ROS}
The algorithm has been developed by using different nodes in order to have a modular \DMC{code}{and flexible design}. 
The nodes \DMC{implemented}{} are:
\begin{itemize}
...
\item a node that, given the states, writes the problem in PDDL\DMC{ syntax}, \DMC{calls}{executes} the Fast Downward \DMC{binary file}{planner} and returns \DMC{a}{the resulting} plan,
